...
const ChatView: React.FC<ChatViewProps> = ({
  ...
  const wsUrl = useMemo(() => `ws://127.0.0.1:8765/ws/notifications/${clientId}`, [clientId])
  ...
  function handleResetWithStorage(agentId?: string) {
    if (agentId) {
      ...
      // 清除当前 Agent 的消�?      setMessages(prev => prev.filter(m => m.agentId !== agentId))
      onReset(agentId)
    } else {
      // 清除所�?Agent 的消�?      agents.forEach(a => clearLocalMessages(a.id))
      setMessages([])
      onReset()
    }
  }
    ...
    <div className="chat-container" style={{ display: 'flex', height: '100%' }}>
      ...
      <aside className="chat-sidebar" style={{
        ...
        <div style={{
          ...
          >
            🧠 {memragEnabled ? t.chat.memragOn : t.chat.memragOff}
          </Button>
        </div>
      </aside>
