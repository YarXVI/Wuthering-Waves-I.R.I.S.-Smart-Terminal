...
const MeetingView: React.FC<MeetingViewProps> = ({ agents }) => {
  ...
  // WebSocket setup
  const wsUrl = activeId ? `ws://127.0.0.1:8765/ws/${activeId}` : null
  ...
  const openExportDlg = async () => {
    if (!activeId) return
    try {
      const res = await fetch(`http://127.0.0.1:8765/meetings/${activeId}/export`)
        ...
        <div className="sidebar-actions" style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6, borderBottom: '1px solid var(--nb-border)' }}>
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            ...
            <button style={{ padding: '4px 8px', borderRadius: 6, fontSize: 10, cursor: 'pointer', background: 'transparent', color: 'var(--nb-text-muted)', border: '1px dashed var(--nb-border-strong)', fontFamily: 'inherit' }}
              onClick={() => setShowTplDlg(true)}>⚙️</button>
          </div>
        </div>
          ...
          {filteredRooms.map(r => (
            ...
            >
              ...
              <div className="room-meta">
                {r.consensus ? '�? : r.status === 'completed' ? '✔️' : '�?} {r.round_count || 0} {t.meeting.rounds} · {r.entry_count || 0} {t.meeting.entries}
              </div>
      ...
      <div className="chat-area">
        <header className="chat-header">
          <span>
            ...
            <div className="chat-agent-title">
              {activeId ? `${rounds > 0 ? `${t.meeting.rounds} ${rounds} · ` : ''}${entries.length} ${t.meeting.entries}` : t.meeting.noMeetings}
            </div>
          </span>
        ...
        {activeId && (
          <div className="meeting-actions">
            <button className="meeting-btn" onClick={handleDiscuss} disabled={running}>
              {running ? `�?${t.meeting.running}` : `▶️ ${t.meeting.discuss}`}
            </button>
