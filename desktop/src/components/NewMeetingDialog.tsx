...
Return JSON: {"suggested": ["agent_id1", "agent_id2"], "reasoning": "..."}`
    try {
      const res = await fetch('http://127.0.0.1:8000/meetings/suggest-agents', {
        method: 'POST',
  ...
  return (
    <div className="dialog">
      ...
      >
        ...
        {templates.map(t => (
          <option key={t.id} value={t.id}>{t.name} 鈥?{t.description}</option>
        ))}
      </select>
      <textarea
        ref={topicInputRef}
      ...
      {suggested.length > 0 && (
        <div className="dialog-agent-select">
          <div className="dialog-desc" style={{ marginTop: 8 }}>
            {t.meeting.recommended}: {reasoning && <span className="dialog-hint"> 鈥?{reasoning}</span>}
          </div>
          {suggested.map(a => (
